...
export default function StorefrontLayout({ children, cart, profile, menuOpen, setMenuOpen, cartOpen, setCartOpen, onUpdateCart, onRemoveCart }) {
  ...
  const search = (event) => { event.preventDefault(); const query = new FormData(event.currentTarget).get('q')?.trim(); navigate(query ? `/product?q=${encodeURIComponent(query)}` : '/product'); setMenuOpen(false) }
  const isAdminOrManager = profile && (profile.role === 'ADMIN' || profile.role === 'MANAGER')
  return <div className="kg-storefront">
    <header className="kg-header"><div className="kg-header-top"><Link to="/home" className="kg-brand" aria-label="KhangGear home"><img src={`${assetBase}assets/khanggear-logo.png`} alt="KhangGear" /></Link><form className="kg-search" onSubmit={search}><Search size={19} /><input name="q" aria-label={text.searchAria} placeholder={text.searchPlaceholder} /><button type="submit">{text.search}</button></form><div className="kg-header-actions">{profile ? (isAdminOrManager ? <a className="kg-account-link" href={`${apiBase}/admin/category/list`} title="Trang Quản trị"><UserRound size={19} /><span>{profile.fullName || profile.username}</span><ChevronDown size={15} /></a> : <a className="kg-account-link" href={`${apiBase}/profile`} title="Hồ sơ cá nhân">{profile.avatarUrl ? <img src={profile.avatarUrl} alt="Avatar" style={{ width: 26, height: 26, borderRadius: '50%', objectFit: 'cover', border: '1.5px solid #cbd5e1', flexShrink: 0 }} /> : <UserRound size={19} />}<span>{profile.fullName || profile.username}</span><ChevronDown size={15} /></a>) : <a className="kg-account-link" href={`${apiBase}/login`}><LogIn size={19} /><span>{text.login}</span></a>} {profile && <a className="kg-logout-button" href={`${apiBase}/logout`} title={text.logout}><LogOut size={18} /><span>{text.logout}</span></a>}<button className="kg-cart-button" onClick={() => setCartOpen(true)} aria-label={text.openCart}><ShoppingBag size={21} />{cart.itemCount > 0 && <b>{cart.itemCount}</b>}</button></div><button className="kg-mobile-menu" onClick={() => setMenuOpen(!menuOpen)} aria-label={text.openMenu}>{menuOpen ? <X /> : <Menu />}</button></div>
      <nav className={`kg-nav ${menuOpen ? 'kg-nav-open' : ''}`}><NavLink to="/home" onClick={() => setMenuOpen(false)}>{text.home}</NavLink><NavLink to="/product" onClick={() => setMenuOpen(false)}>{text.products}</NavLink><Link to="/home#categories" onClick={() => setMenuOpen(false)}>{text.categories}</Link>{isAdminOrManager ? <a href={`${apiBase}/admin/category/list`} onClick={() => setMenuOpen(false)}>Trang Quản trị</a> : (profile && <a href={`${apiBase}/profile`} onClick={() => setMenuOpen(false)}>Hồ sơ</a>)}{profile && <NavLink to="/account/orders" onClick={() => setMenuOpen(false)}>{text.orders}</NavLink>}</nav></header>
