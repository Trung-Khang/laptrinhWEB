...
const initialFilters = { q: '', categoryId: '', minPrice: '', maxPrice: '', inStock: false, sort: 'newest', page: 1, size: 6 }
const text = {
...
function Pagination({ page, totalPages, onChange }) { if (totalPages <= 1) return null; return <nav className="kg-pagination" aria-label={text.pagination}><button disabled={page <= 1} onClick={() => onChange(page - 1)}>Trước</button>{Array.from({ length: totalPages }, (_, index) => index + 1).map((number) => <button className={number === page ? 'kg-current-page' : ''} key={number} onClick={() => onChange(number)}>{number}</button>)}<button disabled={page >= totalPages} onClick={() => onChange(page + 1)}>Sau</button></nav> }
