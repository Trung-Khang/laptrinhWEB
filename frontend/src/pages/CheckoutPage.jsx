...
export default function CheckoutPage() {
  const { cart, profile, refreshCart, notify } = useCart(); const [form, setForm] = useState({ fullName: '', phone: '', email: '', address: '', note: '', paymentMethod: 'COD' }); const [submitting, setSubmitting] = useState(false); const [result, setResult] = useState(null); const [, setFieldErrors] = useState({})
  ...
  const submit = async (event) => { event.preventDefault(); setSubmitting(true); setFieldErrors({}); try { const order = await storefrontApi.checkout(form); await refreshCart(); setResult(order) } catch (error) { setFieldErrors(error.fieldErrors || {}); notify(error.message) } finally { setSubmitting(false) } }
